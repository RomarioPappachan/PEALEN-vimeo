...
export default function AddIntroMaterials({
  videoIndex,
  setIsAddIntroMaterialOpen,
}) {
  const { introVideos, setIntroVideoDetails } = useCreateIntroVideosStore();
  const video = introVideos[videoIndex];

  const handleFileChange = (e, index) => {
    const name = e.target.name;
    const file = e.target.files[0];
    if (file) {
      setIntroVideoDetails(name, file, index);
    }
  };

  const handleRemoveFile = (index) => {
    setIntroVideoDetails("pdf", null, index);
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 flex justify-center items-center px-2 py-6">
      <div className="relative w-full max-w-2xl h-auto max-h-[90vh] bg-[var(--background-primary)] rounded-2xl overflow-hidden flex flex-col items-center pb-8">
        ...
        <button
          type="button"
          onClick={() => setIsAddIntroMaterialOpen(false)}
