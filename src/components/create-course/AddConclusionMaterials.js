...
export default function AddConclusionMaterials({
  id,
  videoIndex,
  setIsAddConclusionMaterialOpen,
}) {
  const { conclusionVideos, setConclusionVideoDetails } =
    useCreateConclusionVideosStore();

  const video = conclusionVideos[videoIndex];

  const handleFileChange = (e, index) => {
    const name = e.target.name;
    const file = e.target.files[0];
    if (file) {
      setConclusionVideoDetails(name, file, index);
    }
  };

  const handleRemoveFile = (index) => {
    setConclusionVideoDetails("pdf", null, index);
  };
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 flex justify-center items-center px-2 py-6">
      <div className="relative w-full max-w-2xl h-auto max-h-[90vh] bg-[var(--background-primary)] rounded-2xl overflow-hidden flex flex-col items-center pb-8">
        ...
        <button
          type="button"
          onClick={() => setIsAddConclusionMaterialOpen(false)}
        ...
        <div className="flex flex-col items-center justify-center px-6 py-6 w-full">
          ...
          <input
            ...
            id={`pdf-${id}`}
            accept=".pdf"
            onChange={(e) => handleFileChange(e, videoIndex)}
            className="hidden"
          />
          ...
          {video?.pdf?.name && (
            <div className="mt-5 flex flex-col items-center space-y-3">
              <p className="text-center text-sm md:text-base text-[var(--text-secondary)] break-words max-w-xs md:max-w-md lg:max-w-lg">
                Selected File:{" "}
                <span className="font-medium">{video?.pdf?.name}</span>
              </p>
              <button
                type="button"
                onClick={() => handleRemoveFile(videoIndex)}
