...
export default function IntroVideo({ videoIndex, setIsAddIntroVideoOpen }) {
  const { introVideos, addIntroVideoIds, removeMuxVideo } =
    ...
    const poll = async () => {
      try {
        ...
        if (playbackId) {
          ...
          addIntroVideoIds(videoIndex, muxUploadId, playbackId, muxAssetId);
  ...
  const handleDeleteMuxVideo = async (muxAssetId) => {
    try {
      const res = await removeMuxVideo(videoIndex, muxAssetId);
      console.log(res);
    } catch (error) {
      console.log(error);
    }
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 dark:bg-white/40 flex justify-center items-center px-2 py-6">
      <div className="relative w-full max-w-3xl min-h-[400px] max-h-[90vh] bg-[var(--background-primary)] rounded-2xl overflow-hidden flex flex-col">
        ...
        <button
          type="button"
          onClick={() => setIsAddIntroVideoOpen(false)}
        ...
        {introVideos[videoIndex]?.videoUrl ? (
          <ClassVideoPlayer
            key={introVideos[videoIndex]?.videourl}
            playbackId={introVideos[videoIndex]?.videoUrl}
            muxAssetId={introVideos[videoIndex]?.muxAssetId}
