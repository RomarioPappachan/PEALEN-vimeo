...
export default function AnimationVideo({ videoIndex, setIsAddAnimationOpen }) {
  ...
  const handleDeleteMuxVideo = async (muxAssetId) => {
    try {
      const res = await removeMuxVideo(videoIndex, muxAssetId, "animation");
      console.log(res);
    } catch (error) {
      console.log(error);
    }
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 dark:bg-white/40 flex justify-center items-center px-2 py-6">
      <div className="relative w-full max-w-3xl min-h-[400px] max-h-[90vh] bg-[var(--background-primary)] rounded-2xl overflow-hidden flex flex-col">
        ...
        {classVideos[videoIndex]?.animationUrl ? (
          <ClassVideoPlayer
            key={classVideos[videoIndex]?.animationUrl}
            playbackId={classVideos[videoIndex]?.animationUrl}
            muxAssetId={classVideos[videoIndex]?.muxAnimationAssetId}
