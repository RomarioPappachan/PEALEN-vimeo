...
export default function ConclusionVideo({
  videoIndex,
  setIsAddConclusionVideoOpen,
}) {
  const { conclusionVideos, addConclusionVideoIds, removeMuxVideo } =
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 dark:bg-white/40 flex justify-center items-center px-2 py-6">
      <div className="relative w-full max-w-3xl min-h-[400px] max-h-[90vh] bg-[var(--background-primary)] rounded-2xl overflow-hidden flex flex-col">
        ...
        <button
          type="button"
          onClick={() => setIsAddConclusionVideoOpen(false)}
        ...
        {conclusionVideos[videoIndex]?.videoUrl ? (
          <ClassVideoPlayer
            key={conclusionVideos[videoIndex]?.videourl}
            playbackId={conclusionVideos[videoIndex]?.videoUrl}
            muxAssetId={conclusionVideos[videoIndex]?.muxAssetId}
