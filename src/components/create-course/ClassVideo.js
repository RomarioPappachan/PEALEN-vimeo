...
export default function ClassVideo({ videoIndex, setIsAddClassVideoOpen }) {
  ...
  const handleDeleteMuxVideo = async (muxAssetId) => {
    try {
      const res = await removeMuxVideo(videoIndex, muxAssetId, "main");
      console.log(res);
    } catch (error) {
      console.log(error);
    }
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 bg-black/40 dark:bg-white/40 flex justify-center items-center px-2 py-6">
      <div className="relative w-full max-w-3xl min-h-[400px] max-h-[90vh] bg-[var(--background-primary)] rounded-2xl overflow-hidden flex flex-col">
        ...
        {classVideos[videoIndex]?.videoUrl ? (
          <ClassVideoPlayer
            key={classVideos[videoIndex]?.videourl}
            playbackId={classVideos[videoIndex]?.videoUrl}
            muxAssetId={classVideos[videoIndex]?.muxAssetId}
