...
import ViewVideoSteps from "./ViewVideoSteps";
import ViewMaterial from "./ViewMaterial";
import ViewVideo from "./ViewVideo";

export default function ClassVideoDetail({ videoIndex, video }) {
  const [isClassMaterialOpen, setIsClassMaterialOpen] = useState(false);
  const [isTranscriptOpen, setIsTranscriptOpen] = useState(false);
  const [isStepsOpen, setIsStepsOpen] = useState(false);

  const [isClassVideoOpen, setIsClassVideoOpen] = useState(false);
    ...
    <>
            ...
            <div className="mt-7 flex gap-3.5">
              <div className="flex-1 grid grid-cols-3 gap-5 gap-y-3">
                <button
                  ...
                  onClick={() => setIsClassVideoOpen(true)}
                >
                ...
                <button
                  type="button"
                  className="w-full h-10 p-3 border border-[var(--border-secondary)] rounded-xl outline-none flex justify-center items-center gap-x-2 cursor-pointer"
                  onClick={() => setIsClassMaterialOpen(true)}
                >
                  <span className="text-sm text-[var(--border-secondary)] font-semibold text-center leading-[18px]">
                    Module Material
                  </span>
      ...
      {isClassMaterialOpen && (
        <ViewMaterial
          ...
          onClose={() => setIsClassMaterialOpen(false)}
        />
      )}
      {isTranscriptOpen && (
        <ViewMaterial
          fileType="srt"
          fileLink={video.videoTranscript}
      ...
      {isClassVideoOpen && (
        <ViewVideo
          ...
          onClose={() => setIsClassVideoOpen(false)}
        />
      )}
      ...
      {isAnimationOpen && (
        <ViewVideo
          videoTitle={video.title}
          videoId={video.animationVideoId}
